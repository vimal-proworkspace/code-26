...
import { round2Api, Round2ScoreSummary, DebuggingSubmission, DebuggingProblem } from '../services/round2Api';

interface Round2AdminInspectionProps {
  roundId: string;
}

export const Round2AdminInspection: React.FC<Round2AdminInspectionProps> = ({ roundId }) => {
  const [scores, setScores] = useState<Round2ScoreSummary[]>([]);
  ...
  const [selectedStudent, setSelectedStudent] = useState<Round2ScoreSummary | null>(null);
  const [studentSubmissions, setStudentSubmissions] = useState<DebuggingSubmission[]>([]);
  ...
  const loadData = async () => {
    try {
      ...
      const [scoresData, problemsData] = await Promise.all([
        round2Api.getRound2Scores(roundId),
        round2Api.getAdminProblems(roundId),
      ]);
      ...
      setError(err.message || 'Failed to load Round 2 inspection data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInspectStudent = async (student: Round2ScoreSummary) => {
    setSelectedStudent(student);
    if (!selectedProblemId) return;

    try {
      setLoadingSubmissions(true);
      const subs = await round2Api.getAdminSubmissions(selectedProblemId, student.id);
  ...
  if (loading) {
    return <div style={{ padding: '2rem', color: '#94a3b8' }}>Loading Round 2 scores...</div>;
  }
  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <div>
          <h2 style={styles.title}>Round 2 Student Scores & Submissions</h2>
          <p style={styles.subtitle}>View student performance, fixed bug counts, and historical C code submissions.</p>
        </div>
        {problems.length > 0 && (
          <div>
        ...
        {scores.length === 0 ? (
          ...
          scores.map((st) => (
            <div key={st.id} style={styles.tableRow}>
              <span style={{ fontWeight: 700, color: '#38bdf8' }}>{st.studentId}</span>
              ...
              <span style={{ color: '#eab308', fontWeight: 700 }}>{st.fixedBugsCount} Bugs</span>
              <span style={{ fontWeight: 700, color: '#38bdf8' }}>{st.score} / {st.maximumScore}</span>
                ...
                {studentSubmissions.map((sub) => (
                  <div key={sub.id} style={styles.subCard}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#94a3b8', marginBottom: '0.5rem' }}>
                      <span>Submission #{sub.submissionNumber}</span>
                      <span>{new Date(sub.submittedAt).toLocaleString()}</span>
                      <span style={{ color: sub.compileStatus === 'SUCCESS' ? '#22c55e' : '#ef4444', fontWeight: 700 }}>
                        {sub.compileStatus}
                      </span>
                    </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  tableHeader: { display: 'grid', gridTemplateColumns: '1.5fr 2fr 1fr 1fr 1fr 1fr 1fr', padding: '0.75rem 1rem', backgroundColor: '#1e293b', color: '#94a3b8', fontSize: '0.85rem', fontWeight: 600 },
  tableRow: { display: 'grid', gridTemplateColumns: '1.5fr 2fr 1fr 1fr 1fr 1fr 1fr', padding: '0.75rem 1rem', borderBottom: '1px solid #1e293b', alignItems: 'center', fontSize: '0.9rem' },
  btnInspect: { backgroundColor: '#0284c7', color: '#fff', border: 'none', padding: '0.3rem 0.75rem', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer' },
  ...
  codePreview: { margin: 0, backgroundColor: '#030712', padding: '0.75rem', borderRadius: '6px', fontFamily: 'monospace', fontSize: '0.85rem', color: '#38bdf8', overflowX: 'auto' },
};
