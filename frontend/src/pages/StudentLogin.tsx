...
export const StudentLogin: React.FC = () => {
  ...
  const { loginStudent, clearError } = useAuth();
    ...
    <div style={{ display: 'flex', minHeight: '100vh', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0f172a', color: '#f8fafc', padding: '1rem' }}>
      <div style={{ width: '100%', maxWidth: '420px', backgroundColor: '#1e293b', padding: '2.5rem', borderRadius: '1rem', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.5), 0 8px 10px -6px rgba(0, 0, 0, 0.5)', border: '1px solid #334155' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '1rem' }}>Student Portal Login</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            disabled={submitting}
            style={{ width: '100%', padding: '0.875rem', backgroundColor: submitting ? '#0284c7' : '#0284c7', color: '#ffffff', fontWeight: 600, borderRadius: '0.5rem', border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', fontSize: '1rem', transition: 'background-color 0.2s' }}
          >
            {submitting ? 'Authenticating...' : 'LOGIN'}
          </button>
        </form>

        <div style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid #334155', display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
