...
export class CExecutor extends BaseExecutor {
  ...
  public async execute(request: CodeExecutionRequest): Promise<ExecutionResult> {
    ...
    try {
      // 1. Write C source code to temporary directory
      await fs.promises.writeFile(sourceFilePath, request.sourceCode, 'utf-8');

      // 2. Compile source code with gcc
      const compileResult = await this.runProcess('gcc', ['-O2', 'solution.c', '-o', binaryName], {
        cwd: tempDir,
        timeLimitMs: 10000, // 10s compilation timeout limit
      });
